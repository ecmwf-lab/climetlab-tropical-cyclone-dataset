...
def get_annotation_csv(annotation_file, data_dir):
    ...
    for each in filname_list:
        ...
        path_list_name = 'ssd_' + str(path_list_name[0]) + str(path_list_name[1]) \
                        + str(path_list_name[2]) + '_' + str(time)
    ...
    df = pd.DataFrame({'path': path_series, 'xmin':xmin_series, 'ymin':ymin_series,\
                     'xmax':xmax_series, 'ymax':ymax_series, 'labels': labels_series})
